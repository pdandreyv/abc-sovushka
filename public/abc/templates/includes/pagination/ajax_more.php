...
//если есть пагинатор
if (count($list)>1) {
	$pagination = '<div class="pagination_ajax"><div class="pagination">';
	if ($n>=$quantity) $pagination.= '';
	else $pagination.= '<a class="button" href="'.pagination_link ('n',$n+1,1).'">'.i18n('common|pagination_next').'</a>';
	$pagination.= '</div></div>';
	//обертка для аджакс
	$pagination = '{content}'.$pagination;
	//v1.2.31 - следующая
	if ($n<$quantity) {
		$abc['page']['next'] = pagination_link ('n',$n+1,1);
	}
	//v1.2.31 - предыдущая
	if ($n>1 AND $n<=count($list)) {
		$abc['page']['prev'] = pagination_link ('n',$n-1,1);
	}
}
//если нет результата
