  ...
  logoutButtons.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      openModal({
        ...
        showActions: true,
        onConfirm: () => {
          // В демо просто возвращаем на страницу входа/регистрации
...
function initProfileOnboardingPage() {
  ...
  // При загрузке страницы: если профиль уже сохранён — показываем режим "только чтение"
  ...
  // Смена пароля (демо): просто показываем успешный toast
  if (passwordForm) {
    passwordForm.addEventListener('submit', (e) => {
      ...
      // В реальном проекте здесь будет запрос к серверу (API) для смены пароля.
      // Сейчас — демо.
